...
const newUploadFile = `export async function uploadFile(
  ...
  for (let i = 1; i <= totalChunk; i++) {
    ...
    try {
      ...
      if (!uploadFileId && lastDecData.fileId) {
        uploadFileId = lastDecData.fileId;
        console.log(\`[ZaloSend] Extracted fileId=\${uploadFileId} from chunk 1\`);
      }
    } catch (err: any) {
  ...
  if (lastResData?.error_code === 0) {
    const finalFileId = lastDecData.fileId || lastDecData.msgId || lastDecData.photoId || uploadFileId;
    let fileUrl = '';
