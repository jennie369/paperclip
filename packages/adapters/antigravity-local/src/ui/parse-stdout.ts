...
// agy print mode emits PLAIN TEXT (not stream-json), so most lines fall through
// to the `{kind:"stdout"}` branch. The JSON branches are kept as a tolerant
// fallback in case a future agy build emits structured events.
