  ...
  return (
    <>
      <div className="flex justify-between items-center mb-6">
        <h3 className="font-bold text-sm text-gem-text uppercase tracking-wider">Customer 360</h3>
        <button type="button" onClick={onEdit} className="text-gem-primary text-xs font-bold hover:underline">
          Sửa
        </button>
      </div>
      ...
      <div className="pcard p-4 mb-6" style={{ borderRadius: 16 }}>
        {crm.company && (
          <div className="mb-4">
            <div className="text-[10px] font-bold text-gem-text-muted uppercase tracking-wider mb-1">Company</div>
        ...
        {(crm.phone || crm.email) && (
          <div className="grid grid-cols-2 gap-3 mb-4">
            {crm.phone && (
              <div>
                <div className="text-[10px] font-bold text-gem-text-muted uppercase mb-1">Số Điện Thoại</div>
                <div className="text-xs font-semibold bg-gem-surface-raised p-1.5 rounded-lg border border-gem-border/5 text-gem-text">{crm.phone}</div>
              </div>
            )}
            {crm.email && (
              <div className="min-w-0">
                <div className="text-[10px] font-bold text-gem-text-muted uppercase mb-1">Email</div>
                <div className="text-xs font-semibold bg-gem-surface-raised p-1.5 rounded-lg border border-gem-border/5 text-gem-text truncate" title={crm.email}>
                  {crm.email}
                </div>
              </div>
            )}
        ...
        <div className="mb-4">
          <div className="flex justify-between mb-1">
            <div className="text-[10px] font-bold text-gem-text-muted uppercase tracking-wider">Lifetime Value</div>
            {crm.ltvDeltaLabel && (
              <div className="text-gem-success text-[10px] font-bold bg-gem-success/10 px-1.5 py-0.5 rounded border border-gem-success/20">{crm.ltvDeltaLabel}</div>
            )}
          </div>
          <div className="text-xl font-black text-gem-text">{crm.ltv}</div>
        </div>
          ...
          <div className="mb-4 pt-4 border-t border-gem-border/10">
            ...
            <div className="flex gap-2">
              ...
              <select
                ...
                className="flex-1 bg-gem-surface border border-gem-border/20 rounded-md px-2 py-1.5 text-xs font-bold text-gem-warning outline-none"
              >
              ...
              <button
                ...
                className="shrink-0 px-2 py-1.5 bg-gem-primary/20 text-gem-primary font-bold text-[10px] rounded-md border border-gem-primary/30 hover:bg-gem-primary hover:text-white transition-colors"
              >
        ...
        {tags.length > 0 && (
          <div className="mb-4 pt-4 border-t border-gem-border/10">
            <div className="text-[10px] font-bold text-gem-text-muted uppercase tracking-wider mb-2">Phân loại (Tags)</div>
            <div className="flex flex-wrap gap-1.5">
              {tags.map((t, i) => (
                <span key={i} className={cn("px-2 py-1 rounded text-[10px] font-bold border", TAG_CHIP[t.tone ?? "neutral"])}>
                  {t.label}
                </span>
              ))}
              <button
                type="button"
                onClick={onAddTag}
                className="px-2 py-1 rounded border border-dashed border-gem-border/40 text-gem-text-muted hover:text-gem-text hover:border-gem-text/50 transition-colors text-[10px] flex items-center gap-1"
              >
      ...
      {journey.length > 0 && (
        <div className="mb-6 bg-gem-surface/30 border border-gem-border/10 rounded-xl p-4">
          <div className="text-[10px] font-bold text-gem-text-muted uppercase tracking-wider mb-4 flex items-center gap-2">
          ...
          <div className="relative pl-3.5 border-l-2 border-gem-border/20 flex flex-col gap-4">
            {journey.map((p, i) => (
              <div key={i} className="relative">
                ...
                />
                <div className="text-[10px] text-gem-text-muted mb-0.5 font-medium">
                  {p.when}{" "}
                  <span className="font-bold" style={p.channelColor ? { color: p.channelColor } : undefined}>
                    {p.channelLabel}
                  </span>
                </div>
                <div className={cn("text-sm leading-snug", p.latest ? "text-gem-text font-medium" : "text-gem-text")}>{p.title}</div>
              </div>
            ))}
          </div>
        </div>
