...
export function Sidebar() {
  ...
  const [sectionOrder, setSectionOrder] = useSectionOrder(ALL_SECTIONS);
    ...
    const ITEM_COMPONENTS: Record<string, React.ReactNode> = {
      "/channels/inbox": <SidebarNavItem to="/channels/inbox" label="Hß╗Öp th╞░" icon={Inbox} badge={totalUnread > 0 ? totalUnread : inboxBadge.inbox} badgeTone={totalUnread > 0 ? "danger" : inboxBadge.failedRuns > 0 ? "danger" : "default"} alert={totalUnread > 0 || inboxBadge.failedRuns > 0} />,
      "/channels/settings": <SidebarNavItem to="/channels/settings" label="C├ái ─æß║╖t k├¬nh" icon={Settings} />,
      "/war-room": <SidebarNavItem to="/war-room" label="War Room" icon={Zap} />,
      "/training": <SidebarNavItem to="/training" label="Ph├▓ng Training" icon={Activity} />,
      "/training/history": <SidebarNavItem to="/training/history" label="Lß╗ïch sß╗¡ Training" icon={Activity} />,
      "/training/audit-log": <SidebarNavItem to="/training/audit-log" label="Tool Audit Log" icon={Activity} />,
      "/agents-config/sessions": <SidebarNavItem to="/agents-config/sessions" label="Phi├¬n Agent" icon={Activity} />,
      ...
      "/ops/knowledge-graph": <SidebarNavItem to="/ops/knowledge-graph" label="Mß║»t Thß║ºn CEO" icon={Network} />,
      "/analytics": <SidebarNavItem to="/analytics" label="Ph├ón T├¡ch MXH" icon={BarChart3} />,
      "/cc": <SidebarNavItem to="/cc" label="Tß╗òng quan" icon={LayoutDashboard} end />,
      "/cc/ai-gen": <SidebarNavItem to="/cc/ai-gen" label="AI Tß║ío Nß╗Öi dung" icon={Sparkles} />,
      "/cc/scripts": <SidebarNavItem to="/cc/scripts" label="Kß╗ïch bß║ún" icon={FileText} />,
      "/cc/calendar": <SidebarNavItem to="/cc/calendar" label="Lß╗ïch ─æ─âng b├ái" icon={Calendar} />,
      "/cc/repurpose": <SidebarNavItem to="/cc/repurpose" label="T├íi sß╗¡ dß╗Ñng" icon={RefreshCw} />,
      "/cc/analytics": <SidebarNavItem to="/cc/analytics" label="Thß╗æng k├¬" icon={BarChart3} />,
      "/cc/image-gen": <SidebarNavItem to="/cc/image-gen" label="Tß║ío h├¼nh" icon={Image} />,
      ...
      "/cc/funnels": <SidebarNavItem to="/cc/funnels" label="Phß╗àu chuyß╗ân ─æß╗òi" icon={Filter} />,
      "/cc/settings": <SidebarNavItem to="/cc/settings" label="C├ái ─æß║╖t CC" icon={Settings} />,
      "/ops/sop-registry": <SidebarNavItem to="/ops/sop-engine" label="Registry Marketplace" icon={Database} />,
      "/agents-config": <SidebarNavItem to="/agents-config" label="Agent LLM (ΓåÆ Registry)" icon={Database} textBadge="Deprecated" textBadgeTone="amber" />,
      "/config": <SidebarNavItem to="/config" label="Trung t├óm Cß║Ñu h├¼nh (ΓåÆ Registry)" icon={Settings} textBadge="Deprecated" textBadgeTone="amber" />,
      "/crm": <SidebarNavItem to="/crm" label="Tß╗òng quan CRM" icon={BarChart3} end />,
      "/crm/customers": <SidebarNavItem to="/crm/customers" label="Kh├ích h├áng" icon={UserCircle} />,
      "/crm/tickets": <SidebarNavItem to="/crm/tickets" label="Phiß║┐u hß╗ù trß╗ú" icon={Ticket} />,
      "/crm/orders": <SidebarNavItem to="/crm/orders" label="─É╞ín h├áng" icon={ShoppingBag} />,
      ...
      "/delegations": <SidebarNavItem to="/delegations" label="ß╗ªy quyß╗ün" icon={Share2} />,
      ...
      "/channels/qa": <SidebarNavItem to="/channels/qa" label="─É├ính gi├í & Tr├¡ tuß╗ç" icon={BarChart3} />,
    ...
    const SECTION_LABELS: Record<string, string> = {
      "kenh-chat": "K├¬nh Chat",
      "van-hanh": "Vß║¡n h├ánh",
      "trung-tam-noi-dung": "Trung t├óm Nß╗Öi dung",
      "cau-hinh": "Cß║Ñu h├¼nh",
      "crm": "CRM",
      "work": "Work",
      "company": "Company"
  ...
  return (
    <aside className="w-60 h-full min-h-0 border-r border-border bg-background flex flex-col">
      {/* Top bar: Company name (bold) + Search ΓÇö aligned with top sections (no visible border) */}
